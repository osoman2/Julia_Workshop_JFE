...
function makeGrid(g::Grid)
	rango_abscisas = -g.lim_abs:g.tam_espacio:g.lim_abs
    rango_ordenadas = -g.lim_ord:g.tam_espacio:g.lim_ord
	M = zeros(Complex, size(rango_ordenadas,1),size(rango_abscisas,1))
	for i in 1:size(M,2)
		for j in 1:size(M,1)
			M[j,i] = rango_abscisas[i] + rango_ordenadas[j]*im 
		end
	end
	return M	
